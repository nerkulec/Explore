...
const tex = (strings: TemplateStringsArray, ...values: string[]): JSX.Element => {
  return <Provider>{values.reduce((final, value, index) => [
    ...final, <Node formula={value} inline/>, <span>{strings[index+1]}</span>], [<span>{strings[0]}</span>] as JSX.Element[])}
  </Provider>
}
